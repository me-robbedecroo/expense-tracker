...
import { getAllWeeks, formatWeekRange, getWeeklyLimit } from '../utils/storage';

export default function OverviewScreen({ navigation }) {
    const [weeks, setWeeks] = useState([]);
    const [refreshing, setRefreshing] = useState(false);
    const [totals, setTotals] = useState({ saved: 0, extra: 0 });
    ...
    const loadWeeks = async () => {
        ...
        // Calculate saved and extra amounts
        let saved = 0;
        let extra = 0;
        allWeeks.forEach((week) => {
            if (week.total < weeklyLimit) saved += weeklyLimit - week.total;
            if (week.total > weeklyLimit) extra += week.total - weeklyLimit;
        });
        setTotals({ saved, extra });
    };
